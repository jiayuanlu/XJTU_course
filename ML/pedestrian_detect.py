...
from math import e, log,exp,sqrt
...
def load_data_set(logger):
    ...
    # 提取负样本
    neg_dir = os.path.join(pwd, 'INRIAPerson/train_64x128_H96/neg')
    ...
    # 提取测试集
    test_dir = os.path.join(pwd, 'INRIAPerson/Test/both')
...
def test_hog_detect(test, svm_detector, logger,test_path):
    '''
    导入测试集，测试结果
    :param test: 测试数据集
    ...
    pwd = os.getcwd()
    test_dir = os.path.join(pwd, test_path)
    ...
    for f in test:
        ...
        rects, weight = hog.detectMultiScale(img, winStride=(4,4), padding=(8,8), scale=1.05)
        w1=list(chain.from_iterable(weight))
        if w1:
            weight1=max(w1)
        else:
            weight1=0
        ...
        if ((name[0]=='c')or(name[0]=='p'))and(name[3]!='f'):
            true.append(1)
        else:
            true.append(0)
    ...
    acc=metrics.roc_auc_score(true,pred)
    print(acc)
...
if __name__ == '__main__':
    ...
    test_hog_detect(test, svm_detector, logger,test_path='INRIAPerson/Test/both')
