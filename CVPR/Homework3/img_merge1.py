...
MinMatchNum = 20
#读取照片
L = cv2.imread('13.jpg')          # queryImage
R = cv2.imread('14.jpg')          # trainImage
#高斯滤波
...
# 将图片结果进行显示
    scal = 0.7 #蹄片显示比例控制
    cv2.imwrite('connect1.jpg',cv2.resize(match, (0, 0), fx=scal, fy=scal, interpolation=cv2.INTER_NEAREST))
    cv2.imwrite('LeftAndRightg1.jpg',cv2.resize(LeftAndRight, (0, 0), fx=scal, fy=scal, interpolation=cv2.INTER_NEAREST))
